...
#else
    const float2 position = TransformWorldToObject(inputData.positionWS);
#endif
...
half4 UniversalFragment_DSTRM(InputData inputData, SurfaceData surfaceData, float2 uv)
{
    ...
    #ifdef _DBUFFER
    // Inlined `ApplyDecalToBaseColor`.
    FETCH_DBUFFER(DBuffer, _DBufferTexture, int2(inputData.positionCS.xy));
    DecalSurfaceData decalSurfaceData;
    DECODE_FROM_DBUFFER(DBuffer, decalSurfaceData);
    color.xyz *= color.xyz * decalSurfaceData.baseColor.w + decalSurfaceData.baseColor.xyz * mainLight.shadowAttenuation * mainLight.distanceAttenuation;
    #endif
